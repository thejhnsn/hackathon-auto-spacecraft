...
class Spacecraft(gym.Env):
    ...
    INITIAL_DATA_STORAGE= 60_000_000        #[bits]
...
if __name__ == "__main__":
    ...
    ecc_prev = 0
    for t in count():
        observation, reward, terminated, truncated, _ = env.step(mode)
        print(observation[3] - ecc_prev)
        ecc_prev = observation[3]
        th_step -= 1
        if(th_step < 200):
            mode = 1
        if(th_step == 0):
